...
export interface LuminousScrollThreadProps {
  /**
   * IDs of section elements the thread will react to on intersection.
   * Particle bursts fire when a new section enters the viewport.
   * @default []
   */
  sectionIds?: string[];

  /**
   * Colour stops for the thread gradient (top → bottom).
   * Each stop: [offset%, color].
   * @default cyan/indigo palette
   */
  gradientStops?: Array<[string, string]>;

  /**
   * Colours used for particle bursts and splash particles.
   */
  particleColors?: string[];

  /**
   * 3 bands of the page where the thread dims to simulate passing behind a card.
   * Each zone: [yStart fraction, yEnd fraction] (0 = top, 1 = bottom).
   * @default [[0.17, 0.25], [0.38, 0.46], [0.59, 0.67]]
   */
  depthZones?: Array<[number, number]>;

  /**
   * Whether to show the bottom splash (ripple + particles) when the thread
   * reaches the end of the page. Re-fires each time the user scrolls to the
   * bottom and comes back.
   * @default true
   */
  splash?: boolean;

  /**
   * z-index of the SVG container. Default 2 works with mix-blend-mode: screen.
   * @default 2
   */
  zIndex?: number;
}
...
const LuminousScrollThread: React.FC<LuminousScrollThreadProps> = ({
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (!pending) {
        pending = true;
        requestAnimationFrame(() => { setScrollProgress(scrollRef.current); pending = false; });
      }
    };
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(entries => {
      ...
      if (id === prevSectionRef.current) return;
      ...
      const normY     = Math.min(1, Math.max(0, midY / dims.h));
  ...
  // ── Pulse animation (direct DOM — zero React re-renders per frame) ───────────
  useEffect(() => {
  ...
  // ── Bottom splash ────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!showSplash || reducedMotion.current || pathLen === 0) return;
    if (scrollProgress < 0.90) { splashFiredRef.current = false; return; }

    if (scrollProgress >= 0.97 && !splashFiredRef.current) {
  ...
  }, [scrollProgress, dims, pathLen, showSplash, particleColors]);

  // ── Render ───────────────────────────────────────────────────────────────────
  const { w, h } = dims;
  const pathD    = buildPath(w, h);
  const fallback = 10000;
  const dashOff  = pathLen > 0 ? pathLen * (1 - scrollProgress) : fallback;
  const mobile   = w < 480;
    ...
    >
      ...
      >
        <defs>
          ...
          <mask id={`lst-mask-${uid}`}>
            ...
            {depthZones.map(([ys, ye], i) => (
              <rect
                key={i}
                x="0" y={h * ys} width={w} height={h * (ye - ys)}
        ...
        <path
          ...
          strokeWidth={mobile ? 10 : 16} strokeLinecap="round"
          strokeDasharray={pathLen || fallback}
          strokeDashoffset={dashOff}
          style={{ filter: 'blur(14px)' }}
        />

        {/* Core thread with depth mask */}
        <path
          ref={pathRef}
          d={pathD} fill="none"
          ...
          <>
            {splash.rings.map((ring, i) => (
              <circle key={`r-${splash.id}-${i}`} cx={splash.x} cy={splash.y}
                r="3" fill="none" stroke={ring.color} strokeWidth="1.8"
                filter={`url(#lst-soft-${uid})`}>
                <animate attributeName="r"            from="3"    to={ring.maxR} dur="1.5s" begin={`${ring.delay}s`} fill="freeze" />
                <animate attributeName="opacity"      from="0.85" to="0"        dur="1.5s" begin={`${ring.delay}s`} fill="freeze" />
                <animate attributeName="stroke-width" from="2"    to="0.4"      dur="1.5s" begin={`${ring.delay}s`} fill="freeze" />
              </circle>
            ))}
            <circle cx={splash.x} cy={splash.y} r="6" fill="#67e8f9" filter={`url(#lst-pulse-${uid})`}>
              <animate attributeName="r"       from="6"    to="22" dur="0.4s" fill="freeze" />
              <animate attributeName="opacity" from="0.95" to="0"  dur="0.4s" fill="freeze" />
            </circle>
            {splash.particles.map((p, i) => (
              <circle key={`sp-${splash.id}-${i}`} cx={splash.x} cy={splash.y}
                ...
                <animate attributeName="opacity" from="0.9" to="0"          dur={`${p.dur}s`} fill="freeze" />
